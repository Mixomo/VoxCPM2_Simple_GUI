...
class VoxCPM2Model(nn.Module):
    ...
    def optimize(self, disable: bool = False):
        ...
        try:
            ...
            self.base_lm.forward_step = torch.compile(self.base_lm.forward_step, mode="reduce-overhead", fullgraph=True, options={"cudagraphs": False})
            self.residual_lm.forward_step = torch.compile(
                self.residual_lm.forward_step, mode="reduce-overhead", fullgraph=True, options={"cudagraphs": False}
            )
            self._feat_encoder_raw = self.feat_encoder
            self.feat_encoder = torch.compile(self.feat_encoder, mode="reduce-overhead", fullgraph=True, options={"cudagraphs": False})
            self.feat_decoder.estimator = torch.compile(
                self.feat_decoder.estimator, mode="reduce-overhead", fullgraph=True, options={"cudagraphs": False}
            )
